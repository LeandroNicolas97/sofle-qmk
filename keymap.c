 ...
 */
  ...
  QK_BOOT  , XXXXXXX,KC_QWERTY,KC_QWERTY,CG_TOGG,XXXXXXX,                     XXXXXXX, XXXXXXX, XXXXXXX, UG_TOGG, XXXXXXX, XXXXXXX,
  XXXXXXX , XXXXXXX,CG_TOGG, XXXXXXX,    XXXXXXX,  XXXXXXX,                     XXXXXXX, KC_MPRV, KC_MPLY, KC_MNXT, XXXXXXX, XXXXXXX,
  XXXXXXX , XXXXXXX, XXXXXXX, XXXXXXX,    XXXXXXX,  XXXXXXX, XXXXXXX,     XXXXXXX, XXXXXXX, KC_PGDN, KC_MUTE, KC_PGUP, XXXXXXX, XXXXXXX,
                   _______, _______, _______, _______, _______,     _______, _______, _______, _______, _______
  )
};
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    switch (keycode) {
        case UG_TOGG:  // Usando UG_TOGG para cambiar el color (o el keycode que prefieras)
...
layer_state_t layer_state_set_user(layer_state_t state) {
    ...
    switch (get_highest_layer(state)) {
        case 0:  // Capa base (QWERTY)
            rgblight_sethsv(hue, 255, 255);  // Usa el color establecido por UG_TOGG
            break;
...
bool encoder_update_user(uint8_t index, bool clockwise) {
    if (index == 0) {  // Encoder izquierdo (volumen)
        ...
        if (clockwise) {
            tap_code(KC_PGDN);  // Scroll hacia abajo (Page Down)
        } else {
            tap_code(KC_PGUP);  // Scroll hacia arriba (Page Up)
        }
    }
    return false;
}
